...
280. Authenticated ledger pulse + remaining panel weave hooks.

Wire in panels:

```ts
import { emitPanelTelemetry, emitPanelGeometry } from './panelWeaveBridge';
emitPanelGeometry('blend');
emitPanelTelemetry({ coherence: 94.2, neuralEntropy: 12.5 });
```
